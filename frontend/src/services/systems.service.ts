// ══════════════════════════════════════════════════════════════
// SAP Spektra — Systems Service
// ══════════════════════════════════════════════════════════════
//
// Data Source Classification:
//   REAL: getSystems, getSystemById, getSystemMetrics, getSystemBreaches,
...
//   DERIVED: CPU/Memory/Disk (aggregated from Host metrics),
...
const log = createLogger('SystemsService');
...
export const getSystemMetrics = async (id: string, hours = 2) => {
  if (isDemoMode()) { await delay(300); return mockMetrics(); }
  return api.getSystemHostMetrics(id, hours);
};

export const getSystemBreaches = async (id: string, limit = 50) => {
  if (isDemoMode()) {
    await delay(300);
    return id
...
export const getSystemSla = async (id: string) => {
  if (isDemoMode()) {
    await delay(300);
...
export const getServerDeps = async (id: string) => {
...
export const getSystemInstances = async (id: string) => {
...
export const getMetricHistory = async (hostname: string) => {
...
export const getSystemHosts = async (id: string) => {
  if (isDemoMode()) { await delay(200); return getSystemHostsMock(id); }
  try {
    const [hosts, sys] = await Promise.all([
      api.getHosts(id) as Promise<ApiRecord[]>,
      api.getSystemById(id) as Promise<ApiRecord>,
...
export const getSystemMeta = async (id?: string) => {
...
export const getSAPMonitoring = async (id: string) => {
